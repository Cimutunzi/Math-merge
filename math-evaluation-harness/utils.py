...
def construct_prompt(example, data_name, args):
    ...
    if args.prompt_type in ["direct", "cot", "pal", "tool-integrated"]:
        ...
        if args.prompt_type in ["direct", "cot"]:
            if data_name in ["minerva_math", "math", "math_oai", "mmlu_stem", "sat_math", "mathqa", "hungarian_exam", "ScaleQuest-Math", "OpenMathInstruct-2", "MATH-500","math-merge","aime24","amc23","olympiadbench","minerva-math"]:
                context = f"Problem:\n{example['question']}\nSolution:"
            else:
    ...
    # SFT models
    elif args.prompt_type in ['qwen_box']:
        full_prompt = f"<|im_start|>system You are a helpful assistant.<|im_end|>\n<|im_start|>user {example['question']} Please reason step by step, and put your final answer within boxed{{}}.\n<|im_end|> <|im_start|>assistant\n"
    ...
    elif args.prompt_type == "step_cot":
        full_prompt = (
            'Below is a math problem with part of the thought process. Please continue the reasoning and complete the solution.\n'
            '### Problem:\n'
            '{question}\n'
            '### Solution:'
        )
        full_prompt = full_prompt.format(question=example['question'])
