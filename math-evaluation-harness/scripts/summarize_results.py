...
def main():
    ...
    summarize_results(args.result_dir, args.data_names, args.split, args.prompt_type)


def summarize_results(result_dir, data_names, split, prompt_type):
    data_list = data_names.split(',')

    # read the result
    results = []
    for data_name in data_list:
        files = glob.glob(f"{result_dir}/{data_names}/Recording/{split}_{prompt_type}*metrics.json")
