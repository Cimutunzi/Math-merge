...
def parse_args():
    ...
    parser.add_argument("--prompt_type", default="tool-integrated", type=str, choices=["direct", "cot", "pal", "tool-integrated", "self-instruct", "self-instruct-boxed", "tora", "pal", "cot", "wizard_zs", "platypus_fs", "deepseek-math", "kpmath","qwen_box"])
...
for i, data_name in enumerate(data_list):
    ...
    print(f"CUDA_VISIBLE_DEVICES={','.join(available_gpus[gpu_idx:gpu_idx+args.gpus_per_model])}")
    ...
    if args.adpter_path:
        cmd = f"sleep {gpu_idx * 3} && " \
            ...
            f"--adpter_path {args.adpter_path} " \
...
if __name__ == "__main__":
    ...
    # summarize_results(args.output_dir, args.data_names, args.split, args.prompt_type)
