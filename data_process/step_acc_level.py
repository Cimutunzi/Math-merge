...
# 设置文件路径
input_file = '/data/qq/models/deepseek-math-7b-instruct/math_eval/math-merge/Recording/step_1-2_deepseek-math_seed0_t0.7_n_sample_16.jsonl'  # 评测结果文件路径
data_file = '/data/qq/math-evaluation-harness/data/math-merge/train.jsonl'  # 主数据文件路径
...
with jsonlines.open(data_file) as reader:
    for obj in reader:
        questions_and_solutions[obj['idx']] = {
...
for idx, data in all_data.items():
    if idx in questions_and_solutions:
        # 整合每一条数据
        question_solution = questions_and_solutions[idx]
        processed_data.append({
            'idx': idx,
            'question': question_solution['question'],
            'solution': question_solution['solution'],
            'level': data['level'],
            'accuracy': data['accuracy']
        })

# 对数据按准确率排序
sorted_data = sorted(processed_data, key=lambda x: x['accuracy'], reverse=True)
